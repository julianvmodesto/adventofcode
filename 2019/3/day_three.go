...
func NewPanels(wire1, wire2 string) (*Panel, *Panel) {
	var instructions1 = strings.Split(wire1, ",")
	var instructions2 = strings.Split(wire2, ",")

	var size = getPanelMaxSize(instructions1)
	var size2 = getPanelMaxSize(instructions2)
	if size2 > size {
		size = size2
	}
	var start = getPanelStart(size)

	var panel1 = setPath(instructions1, size, start)
	var panel2 = setPath(instructions2, size, start)
...
func (p *Panel) GetDistanceToClosestIntersection(p2 *Panel) float64 {
	var intersections = [][]int{}

	for x := 0; x < p.size; x++ {
		for y := 0; y < p.size; y++ {
			if p.panel[x][y] && p2.panel[x][y] {
...
func setPath(instructions []string, size, start int) [][]bool {
	var panel [][]bool = newPanel(size)
	var x, y = start, start
	for _, i := range instructions {
		...
		for ; moves > 0; moves-- {
			...
			if x >= size {
				log.Fatal("at the end of x")
			}
			if y >= size {
				log.Fatalf("at the end of y")
			}
			panel[x][y] = true
		}
	}
...
func calculateManhattanDistance(x1, y1, x2, y2 float64) float64 {
...
func getPanelMaxSize(instructions []string) int {
	var size int
	for _, i := range instructions {
...
func getPanelStart(size int) int {
	return size / 2
}
...
func main() {
	...
	var wire1, wire2 = NewPanels(in1, in2)
