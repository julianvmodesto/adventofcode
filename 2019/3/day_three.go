...
func GetDistanceToClosestIntersection(wire1, wire2 string) float64 {
	...
	for _, i := range instructions2 {
		...
		for ; moves > 0; moves-- {
			...
			if panel[x][y] {
				var distance = calculateManhattanDistance(0, 0, float64(x), float64(y))
...
func main() {
	...
	var distance = GetDistanceToClosestIntersection(in1, in2)
